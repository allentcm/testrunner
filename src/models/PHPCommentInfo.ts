...
export class PHPCommentInfo {
  constructor(
    public readonly type: PHPCommentType,
    public readonly startLineNumber: number = NaN,
    public endLineNumber: number = NaN
  ) {}
  ...
  rangeToString(): string {
    ...
    if (validStart && validEnd) {
      if (this.startLineNumber === this.endLineNumber) {
        ...
        return (
          "from " +
          (this.startLineNumber + 1).toString() +
          " to " +
          (this.endLineNumber + 1).toString()
        );
      }
    } else {
      return "";
