...
import { IVisualCodeDocumentShim } from "./interfaces/IVisualCodeDocumentShim";
import {
  IVisualCodeQuickPickItem,
  IVisualCodeShim,
} from "./interfaces/IVisualCodeShim";
import { Configuration } from "./Configuration";
import { PHPTestFunctionInfo } from "./PHPTestFunctionInfo";
import { VisualCodeDocumentShim } from "./VisualCodeDocumentShim";

/**
 * This class handles stateful functionality for VSCode workspaces
 */
export class VisualCodeShim
  extends vscode.Disposable
  implements IVisualCodeShim
{
  ...
  protected updateConfiguration(): void {
    ...
    this.configuration.commands.runAllUnitTests =
      config["commands"]["runAllUnitTests"];
    this.configuration.commands.runCodeCoverage =
      config["commands"]["runCodeCoverage"];
    this.configuration.commands.codeCoverageReport =
      config["commands"]["codeCoverageReport"];
    this.configuration.composer.enableInstall =
      config["composer"]["enableInstall"];
    this.configuration.composer.enableNamespace =
      config["composer"]["enableNamespace"];
    this.configuration.composer.packagesRequired =
      config["composer"]["packagesRequired"];
    this.configuration.composer.packagesDevelopment =
      config["composer"]["packagesDevelopment"];
    this.configuration.composer.commands.directory =
      config["composer"]["commands"]["directory"];
    this.configuration.composer.commands.require =
      config["composer"]["commands"]["require"];
    this.configuration.composer.commands.update =
      config["composer"]["commands"]["update"];
    this.configuration.composer.commands.dumpAutoload =
      config["composer"]["commands"]["dumpAutoload"];
  ...
  appendToOutputChannel(text: string) {
    if (!this._output) {
      this._output = vscode.window.createOutputChannel("Test Runner");
    }
  ...
  addDiagnostic(
    ...
    if (!this._diagnostics) {
      this._diagnostics =
        vscode.languages.createDiagnosticCollection("PHP-TDD");
    }
   ...
   */
  async showPickList(
    options: Array<IVisualCodeQuickPickItem>,
    allowMany: boolean = false
  ): Promise<IVisualCodeQuickPickItem | undefined> {
    return vscode.window.showQuickPick<vscode.QuickPickItem>(options, {
      canPickMany: allowMany,
    });
  }
}
