...
export class ComposerSetupService {
  ...
  constructor(ui: IVisualCodeShim, spawnFactory: SpawnServiceFactoryType) {
    ...
    this._cmdComposerDumpAutoload =
      ui.configuration.composer.commands.dumpAutoload;
  ...
  async checkRequirements(
    ...
    developmentPackages: Array<string>
  ) {
    ...
    const substitutionPayload = {
      ...
      __TEST_SUBDIRECTORY__: path
        .normalize(this._testSubdirectory)
        .replace(/\\/g, "/"),
      __PACKAGE__: "",
      __FLAGS__: "",
    };
    ...
    const composer = await this.loadComposerJson(
      path.join(workspaceFolderPath, "composer.json")
    );
    if (composer.require) {
      existingPackages = Object.getOwnPropertyNames(composer.require);
    }
    if (composer["require-dev"]) {
      existingPackages = existingPackages.concat(
        Object.getOwnPropertyNames(composer["require-dev"])
      );
    }
    ...
    if (composerPackages.length > 0) {
      ...
      for (let pkg of composerPackages) {
        this._ui.appendToOutputChannel(
          "*** Adding Composer package " + pkg.package + ", please wait... ***"
        );
        substitutionPayload.__PACKAGE__ = pkg.package;
        substitutionPayload.__FLAGS__ = pkg.development ? "--dev" : "";

        const cmd = PHPUtility.substituteValues(
          this._cmdComposerRequire,
          substitutionPayload
        );
        ...
        await this._spawnFactory(this._ui)
          ...
          .setStartInDirectory(
            PHPUtility.substituteValues(this._dirComposer, substitutionPayload)
          )
          .run();
        added++;
      }
  ...
  async assignNamespace(
    ...
    let modified = false;
    const composerJsonFilePath = path.join(
      workspaceFolderPath,
      "composer.json"
    );
    /* istanbul ignore else */
