...
export class PHPUnitTestBuilderService {
   ...
   */
  async editUnitTestFunction(
    functionName: string,
    useBaseTestCase: boolean,
    ...
    if (fs.existsSync(testFileInfo.filename)) {
      ...
      contents = await this.createTestFile(
    ...
    if (testInfo.exists) {
      const character = document.getLineFirstNonWhitespaceCharacterIndex(
        testInfo.lineNumber
      );
      startPos = endPos = new VisualCodePositionShim(
        testInfo.lineNumber,
        character
      );
    } else {
      let results = await this.addTestFunction(
        functionName,
        testInfo.lineNumber - 1,
        entity,
        document
      );
      startPos = results[0];
    ...
    this._composer.assignNamespace(
      entity.namespace,
  ...
  protected getUnitTestFileNameInfo(documentPath: string) {
    ...
    for (let workspaceFolderPath of this._ui.getWorkspaceFolderPaths()) {
      if (fileDirectory.indexOf(workspaceFolderPath) === 0) {
        ...
        if (idx !== -1) {
          ...
          testFile =
            testFile.substring(0, testFile.length - ext.length) +
            "UnitTestCest.php";
    ...
    throw new Error(
      "Unable to determine unit test directory, make sure that a PHP document is open in a workspace"
    );
  }
   ...
   */
  protected async createTestFile(
    testFileName: string,
    useBaseTest: boolean,
    sourceFileName: string,
    relativeDirctory: string
  ) {
    // Get the name of the template to use
    if (this._unitTestTemplateFile) {
      const templateFile = path.join(
        path.dirname(__dirname),
        this._unitTestTemplateFile
      );
      if (!fs.existsSync(templateFile)) {
        throw new Error(
          "Unable to set up test case: " + templateFile + " does not exist"
        );
      }
      ...
      await new Promise<void>(function (resolve, reject) {
        fs.createReadStream(templateFile)
          .pipe(fs.createWriteStream(testFileName))
          .on("finish", () => {
            resolve();
          })
          .on("error", (err: any) => {
      ...
      let testCaseClassName = PHPUtility.capitalizeFirstLetter(
        path.basename(sourceFileName)
      );
      let cext = path.extname(testCaseClassName);
      if (cext.length > 0) {
        testCaseClassName = testCaseClassName.substring(
          0,
          testCaseClassName.length - cext.length
        );
      }
      testCaseClassName += "UnitTestCest";
   ...
   * @param functionName
   */
  protected async getTestMethodLine(
    entities: Array<PHPEntityInfo>,
   ...
   */
  protected async addTestFunction(
    testFunctionName: string,
    lineNumber: number,
    entity: PHPEntityInfo,
    document: IVisualCodeDocumentShim
  ): Promise<Array<IVisualCodePositionShim>> {
    ...
    while (--backToLineNumber > 0) {
      ...
      if (pastLineText.length > 0) {
        let startAt =
          document.getLineFirstNonWhitespaceCharacterIndex(backToLineNumber);
    ...
    if (entity instanceof PHPFunctionInfo) {
      covers =
        ...
        " * @covers " +
        (entity.class
          ? entity.class.fullName + "::" + entity.name
          : entity.fullName) +
        eol +
        padding +
        " **/";
    ...
    return [
      new VisualCodePositionShim(
        currentLineNumber,
        document.getLineFirstNonWhitespaceCharacterIndex(currentLineNumber)
      ),
      new VisualCodePositionShim(
        currentLineNumber,
        document.getLineTextAt(currentLineNumber).length
      ),
    ];
  }
  ...
  protected async checkNamespace(
    entity: PHPEntityInfo,
    useEntities: Array<PHPUseInfo>,
    document: IVisualCodeDocumentShim
  ) {
    let namespace: string;
    if (
      entity instanceof PHPFunctionInfo &&
      entity.class instanceof PHPClassInfo
    ) {
      if (entity.class.namespace) {
        namespace = entity.class.namespace
          ? entity.class.namespace + "\\" + entity.class.name
          : entity.class.name;
      } else {
