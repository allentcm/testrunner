...
import { PHPCommentInfo, PHPCommentType } from "../models/PHPCommentInfo";
...
export class PHPFileParserService {
  ...
  constructor(
    spawnServiceFactory: SpawnServiceFactoryType,
    ui: IVisualCodeShim
  ) {
  ...
  /**
   * Return the testable entity, if any,
   ...
   */
  async getEntityAtLineNumber(
    phpCode: string,
    evalAtLineNumber: number
  ): Promise<PHPEntityInfo | undefined> {
  ...
  async tokenize(phpCode: string): Promise<Array<any>> {
    ...
    try {
      ...
      if (
        e.message.indexOf("undefined function json_encode") !== -1 &&
        !forceJSON
      ) {
        await this._ui.setEnablePHPExtensions(true);
        return this.tokenize(phpCode);
      } else {
        throw e;
      }
    }
   ...
   * @param tokens
   */
  static getNextDescriptorOnLine(
    ctr: number,
    lineNo: number,
    tokens: Array<any>
  ): string {
   ...
   */
  parse(
    parsedTokens: Array<any>,
    evalAtLineNumber: number = NaN
  ): PHPEntityInfo | Array<PHPEntityInfo> | undefined {
    ...
    while (ctr < limit) {
      ...
      if (Array.isArray(token)) {
        ...
        switch (content) {
          case "namespace":
          case "phpnamespace":
            const namespaceName = PHPFileParserService.getNextDescriptorOnLine(
              ctr,
              lineNumber,
              activeTokens
            );
            ...
            break;
          case "use":
            const namespace = PHPFileParserService.getNextDescriptorOnLine(
              ctr,
              lineNumber,
              activeTokens
            );
            entities.push(
              new PHPUseInfo("", namespace, lineNumber, lineNumber)
            );
            break;
          case "class":
            const className = PHPFileParserService.getNextDescriptorOnLine(
              ctr,
              lineNumber,
              activeTokens
            );
            ...
            break;
          case "function":
            const functionName = PHPFileParserService.getNextDescriptorOnLine(
              ctr,
              token[2],
              activeTokens
            );
            if (functionName.length > 0 && !currentFunction) {
              ...
              currentFunction = new PHPFunctionInfo(
                functionName,
                currentNamespace,
                lineNumber
              );
          ...
          default:
            ...
            if (content.match(/\/\*(\*(?!\/)|[^*])*\*\//)) {
              ...
              currentComment = new PHPCommentInfo(
                PHPCommentType.Block,
                lineNumber,
                lineNumber + lines
              );
              ...
              if (
                currentComment &&
                currentComment.type === PHPCommentType.Inline
              ) {
              ...
              currentComment &&
              !PHPFileParserService.isLineClassOrFunction(
                ctr + 1,
                lineNumber,
                activeTokens
              )
        ...
        switch (token) {
          ...
          case "}":
            if (currentFunction && currentFunction.depth === depth) {
              ...
              var from = currentFunction.comment
                ? currentFunction.comment.startLineNumber
                : currentFunction.startLineNumber;
              if (
                !isNaN(evalAtLineNumber) &&
                evalAtLineNumber >= from &&
              ...
              var from1 = currentClass.comment
                ? currentClass.comment.startLineNumber
                : currentClass.startLineNumber;
              if (
                !isNaN(evalAtLineNumber) &&
                evalAtLineNumber >= from1 &&
